...
class Data:
    ...
    redis_client: Redis
    data: Union[List, bytes, None] = None
...
class Audio(Data):
    def __init__(self, chunk_name: str, redis_client: Redis, data: bytes = None):
        super().__init__(chunk_name=chunk_name, key="audio", redis_client=redis_client, data=data)


class Transcript(Data):
    def __init__(self, chunk_name: str, redis_client: Redis, data: List = None):
