...
async def get_stored_knn(emb:list, client_id, redis_client):
    ...
    if len(search_result)>0:
        similar = list(set([s.payload['speaker_id'] for s in search_result if s.score>0.71]))
        if len(similar)>1: 
            await redis_client.lpush('SimilarSpeakers', json.dumps(similar))

        return search_result[0].payload['speaker_id'], search_result[0].score
...
async def process_speaker_emb(emb:list,redis_client, client_id):
    speaker_id, score = await get_stored_knn(emb, client_id,redis_client)
    print(score)
    if speaker_id:
        if score > 0.95:
            pass
